...
void fn::flip_cluster(sw*** lattice, deque < int > cluster, bool decision, int xc, int zcR) {
  if (decision) {
    for (std::deque < int > ::iterator it = cluster.begin(); it != cluster.end(); ++it) {
	  for (int zc=zcR; zc < Nl; ++zc){
	      lattice[xc][ * it][zc].flip();
	      lattice[xc][ * it][zc].reset_bond(); //Here bond is like site.
	      if (rann() > central.bond_prob(lattice[xc][yc][zc], beta, J3 / M))break;
	  }
	  for (int zc=zcR; zc >= 0; --zc){
	      lattice[xc][ * it][zc].flip();
	      lattice[xc][ * it][zc].reset_bond(); //Here bond is like site.
	      if (rann() > central.bond_prob(lattice[xc][yc][zc], beta, J3 / M))break;
	  }
...
void fn::flip_cluster_intl(sw *** lattice, int yc, bool * decision_list) {
  for (int xc = 0; xc < M; xc++) {
  for (int zc = 0; zc < Nl; zc++) {
    if (decision_list[lattice[xc][yc][zc].cl()]) lattice[xc][yc][zc].flip(); lattice[xc][yc][zc].reset_bond();
  }}
}
...
void fn::swendsen_flip(sw *** lattice, sw* plaquette, double temperature) {
  ...
  /*
    for (int yc = 0; yc < Ns; ++yc) {
      ...
      for (int i = 0; i < no_clusters; i++) flip_decision[i] = (rann() <= 1/(1+exp(-cluster_field[i] / temperature)));
...
deque < int > fn::dip_enrg_diff_cluster(sw *** lattice, sw * plaquette, deque < int > cluster, deque < int > plaq_cluster, double * dE, int xc, int zc) {
...
*/  
  ...
  * dE = E_new - E_old - E_J3; plaq_cluster.clear();
  return plaq_cluster;
}
//************************************ Physical quantities ***************************************************

void
fn::phys_quant(sw *** lattice, double * op, double * op_sq, double * op_four, double * Ic, double * Ic_sq, double * E, double * cv) {
...
fn::pl(sw *** lattice, int xc, int yc, int zc, double * pl_t) {
  ...
  if (yc < Np) {
    ...
    * pl_t /= 4; //pow(-1, ((yc%Np) % lx) + ((yc%Np) / lx)) 
    ...
    for (int b = 0; b < 8; b++) dip(lattice, xc, dp[b], zc, & Px[b], & Py[b]);
    * pl_t = 1 * (Px[0] + Px[1] + Px[2] + Px[3]) - 1 * (Py[0] + Py[1] + Py[2] + Py[3]) + -1 * (Px[4] + Px[5] + Px[6] + Px[7]) + 1 * (Py[4] + Py[5] + Py[6] + Py[7]);
    * pl_t /= 4; //* pl_t *= pow(-1, ((yc%Np) % lx) + ((yc%Np) / lx))         
  }
}
